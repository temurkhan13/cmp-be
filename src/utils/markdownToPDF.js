...
const cleanInline = (html) => {
  if (!html) return "";
  return sanitizeText(html.replace(/<br\s*\/?>/g, " ").replace(/<[^>]*>/g, "")).trim();
};
...
const convertMarkdownToPDF = (markdown, opts = {}) => {
  return new Promise((resolve, reject) => {
    ...
    if (opts.coverPage !== false) {
      ...
      doc.fontSize(28).font(FONTS.bold).fillColor(BRAND.primary).text(title, PAGE.marginLeft, 280, {
        align: "center",
        width: pageWidth,
        lineGap: 4,
      });
    ...
    const renderCodeBlock = () => {
      ...
      const bgY = doc.y - 4;
      doc.save().roundedRect(PAGE.marginLeft, bgY, pageWidth, blockHeight, 4).fill(BRAND.bgLight);
      doc.restore();
    ...
    const renderBlockquote = () => {
      ...
      const bgY = doc.y;
      doc.save().rect(PAGE.marginLeft, bgY, 3, blockHeight).fill(BRAND.accent);
      doc.restore();
      doc
        .save()
    ...
    lines.forEach((rawLine) => {
      ...
      if (line.startsWith("<h1>") || line.startsWith("<h1 ")) {
        ...
        ensureSpace(50);
        drawDivider();
        doc.font(FONTS.bold).fontSize(26).fillColor(BRAND.primary).text(text, { align: "left" });
        doc.moveDown(0.6);
        return;
      }
      if (line.startsWith("<h2>") || line.startsWith("<h2 ")) {
        ...
        doc.moveDown(0.3);
        drawDivider();
        doc.font(FONTS.bold).fontSize(19).fillColor(BRAND.primary).text(text, { align: "left" });
        doc.moveDown(0.4);
        return;
      }
      if (line.startsWith("<h3>") || line.startsWith("<h3 ")) {
        ...
        ensureSpace(30);
        doc.moveDown(0.2);
        doc.font(FONTS.bold).fontSize(15).fillColor(BRAND.textDark).text(text, { align: "left" });
        doc.moveDown(0.3);
        return;
      }
      if (line.match(/^<h[4-6]/)) {
        ...
        ensureSpace(24);
        doc.font(FONTS.bold).fontSize(12).fillColor(BRAND.textMedium).text(text, { align: "left" });
        doc.moveDown(0.3);
        return;
      }
      ...
      if (line.startsWith("<p>") || line.startsWith("<div>")) {
        ...
        if (!text) return;
        ensureSpace(16);
        doc.font(FONTS.regular).fontSize(10.5).fillColor(BRAND.textDark).text(text, {
          width: pageWidth,
          lineGap: 2.5,
          align: "left",
        });
    ...
    for (let i = 0; i < totalPages; i++) {
      ...
      doc.text(`Page ${i + 1} of ${totalPages}`, PAGE.marginLeft, footerY + 6, {
        width: pageWidth,
        align: "right",
        lineBreak: false,
      });
      ...
      // Footer brand
      doc.font(FONTS.regular).fontSize(8).fillColor(BRAND.textLight);
      doc.text("Generated by ChangeAI", PAGE.marginLeft, footerY + 6, {
        width: pageWidth,
        align: "left",
        lineBreak: false,
      });
