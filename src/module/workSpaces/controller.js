...
const extractFileText = catchAsync(async (req, res) => {
  ...
  try {
    ...
    logger.error("Extract text error:", e.message);
...
const assistantChatUpdate = catchAsync(async (req, res) => {
  ...
  if (req.file) {
    ...
    logger.info(`File uploaded to Supabase: ${req.file.originalname} -> ${publicUrl} (${req.file.mimetype})`);
    if (fileType === "image") {
...
const bookmarkMessage = catchAsync(async (req, res) => {
  const { workspaceId, folderId, contextId, contextType, messageId } = req.params;
  const { user } = req;
  const bookmark = await service.bookmarkMessage(
    workspaceId,
    folderId,
    contextId,
    messageId,
...
const unbookmarkMessage = catchAsync(async (req, res) => {
  ...
  const bookmark = await service.unbookmarkMessage(
    workspaceId,
    folderId,
    contextId,
    messageId,
...
const getBookmarksForChat = catchAsync(async (req, res) => {
  ...
  const bookmarks = await service.getBookmarksForChat(
    userId,
    workspaceId,
    folderId,
    contextId,
...
const addReplyToComment = catchAsync(async (req, res) => {
  ...
  body.userName = `${user.firstName || ""} ${user.lastName || ""}`.trim() || "User";
  body.timestamp = new Date();
  const reply = await service.addReplyToComment(
    workspaceId,
    folderId,
    chatId,
    messageId,
...
const updateReplyInComment = catchAsync(async (req, res) => {
  ...
  const reply = await service.updateReplyInComment(
    workspaceId,
    folderId,
    chatId,
    messageId,
...
const deleteReplyFromComment = catchAsync(async (req, res) => {
  const { workspaceId, folderId, chatId, messageId, commentId, replyId } = req.params;
  const result = await service.deleteReplyFromComment(
    workspaceId,
    folderId,
    chatId,
    messageId,
...
const updateBusinessInfo = catchAsync(async (req, res) => {
  const { workspaceId, folderId, businessInfoId } = req.params;
  const { body } = req;
  const businessInfo = await service.updateBusinessInfo(
    workspaceId,
    folderId,
    businessInfoId,
    body
...
const updateAssistantChat = catchAsync(async (req, res) => {
  const { workspaceId, folderId, chatId } = req.params;
  const { body } = req;
  const updatedAssistantChat = await service.updateAssistantChat(
    workspaceId,
    folderId,
    chatId,
    body
...
const moveToTrash = catchAsync(async (req, res) => {
  ...
  if (updatedEntity) {
    return res
      ...
      .json({ message: `${entityType} moved to trash successfully!` });
  }
...
const restoreFromTrash = catchAsync(async (req, res) => {
  ...
  if (updatedEntity) {
    return res
      .status(httpStatus.OK)
      .json({ message: `${entityType} restored from trash successfully!` });
  }
  res.status(httpStatus.NOT_FOUND).json({ message: `${entityType} not found!` });
});
const deleteFromTrash = catchAsync(async (req, res) => {
  ...
  if (updatedEntity) {
    return res
      .status(httpStatus.OK)
      .json({ message: `${entityType} deleted from trash successfully!` });
  }
  res.status(httpStatus.NOT_FOUND).json({ message: `${entityType} not found!` });
});
...
const getUserChats = catchAsync(async (req, res) => {
  const { user } = req;
  const { _id: userId } = user;
  const chats = await service.getUserChats(userId, req.query);
  res.status(httpStatus.OK).send(chats);
});
const _searchChats = catchAsync(async (req, res) => {
  const { user } = req;
...
const getUserAssessments = catchAsync(async (req, res) => {
  const { user } = req;
  const { _id: userId } = user;
  const assessments = await service.getUserAssessments(userId, req.query);
...
const bulkCreateWireframeEntity = catchAsync(async (req, res) => {
  const { workspaceId, folderId, wireframeId } = req.params;
  const { body } = req;
  const wireframe = await service.bulkCreateWireframeEntity(
    workspaceId,
    folderId,
    wireframeId,
    body
...
const bulkUpdateWireframeEntity = catchAsync(async (req, res) => {
  const { workspaceId, folderId, wireframeId } = req.params;
  const { body } = req;
  const wireframe = await service.bulkUpdateWireframeEntity(
    workspaceId,
    folderId,
    wireframeId,
    body
...
const bulkDeleteWireframeEntity = catchAsync(async (req, res) => {
  const { workspaceId, folderId, wireframeId } = req.params;
  const { body } = req;
  const wireframe = await service.bulkDeleteWireframeEntity(
    workspaceId,
    folderId,
    wireframeId,
    body
...
const updateWireframeEntity = catchAsync(async (req, res) => {
  ...
  const { body } = req;
  const wireframe = await service.updateWireframeEntity(
    workspaceId,
    folderId,
    wireframeId,
    entityId,
...
const deleteWireframeEntity = catchAsync(async (req, res) => {
  const { workspaceId, folderId, wireframeId, entityId } = req.params;
  const wireframe = await service.deleteWireframeEntity(
    workspaceId,
    folderId,
    wireframeId,
    entityId
...
const uploadEntityImage = catchAsync(async (req, res) => {
  ...
  const wireframe = await service.uploadEntityImage(
    workspaceId,
    folderId,
    wireframeId,
    entityId,
...
const toggleMessageLike = catchAsync(async (req, res) => {
  ...
  const { user } = req;
  const like = await service.toggleMessageLike(
    workspaceId,
    folderId,
    contextId,
    messageId,
...
const toggleMessageDislike = catchAsync(async (req, res) => {
  const { workspaceId, folderId, contextType, contextId, messageId } = req.params;
  const { user } = req;
  const dislike = await service.toggleMessageDislike(
    workspaceId,
    folderId,
    contextId,
    messageId,
...
const moveChatToFolderOfSameWorkspace = catchAsync(async (req, res) => {
  ...
  const result = await service.moveChatToFolderOfSameWorkspace(
    workspaceId,
    folderId,
    chatId,
    newFolderId
