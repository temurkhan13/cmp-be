...
const createFeedback = async (body) => {
	...
	if (body.category && Array.isArray(body.category)) {
		for (const cat of body.category) {
			const { data: catData, error: catError } = await supabase.from("feedback_categories").insert({
				feedback_id: data.id,
				name: cat.name,
			}).select().single();
...
const updateFeedbackById = async (feedbackId, body) => {
	...
	const { data, error } = await supabase.from("feedback").update(update).eq("id", feedbackId).select().single();
	if (error) throw error;
	return data;
};
