  ...
  if (existing) {
    ...
    if (!hasQA && !hasReport) {
      ...
      const aiPayload = {
        ...
        business_info: businessInfoStr || body.business_info || "",
      ...
      if (aiResponse.status && aiResponse.data?.message) {
        ...
        if (isMarkdownDetected(aiData.message)) {
          await supabase.from("assessment_reports").insert({
            ...
            is_generated: true,
            title: aiData.title || name || "Report Title",
...
const getWorkspaceAssessmentById = async (id) => {
  ...
  const { data: qa } = await supabase
    .from("assessment_qa")
    .select("*, reactions:assessment_message_reactions(*)")
    .eq("assessment_id", id)
    .order("created_at");
  if (report) {
    data.report = {
  ...
  const { data: media } = await supabase
    .from("folder_assessment_media")
    .select("*")
    .eq("assessment_id", id)
    .order("created_at");
  const { data: documents } = await supabase
    .from("folder_assessment_documents")
    .select("*")
    .eq("assessment_id", id)
    .order("created_at");
  const { data: linkData } = await supabase
    .from("folder_assessment_links")
    .select("*")
    .eq("assessment_id", id)
    .order("created_at");
...
const updateAssessmentAnswer = async (workspaceAssessmentId, body) => {
  ...
  if (body.fileBuffer) {
    ...
    try {
      ...
      logger.error("Assessment file parse error:", e.message);
    }
  ...
  const history = [
    "",
    ...(qaHistory || []).flatMap((q) => [q.question, q.answer || ""]),
    answerForAI,
  ];
  ...
  if (!aiResponse.status)
    return handleStatus(
      false,
      "AI service unavailable — your answer was not saved, please try again"
    );
  ...
  if (!aiData?.message)
    return handleStatus(
      false,
      "AI returned empty response — your answer was not saved, please try again"
    );
  ...
  if (answer) {
    ...
    if (foundUrls && foundUrls.length > 0) {
      const linkRows = foundUrls.map((url) => {
        try {
          return { assessment_id: workspaceAssessmentId, name: new URL(url).hostname, url };
        } catch {
          return { assessment_id: workspaceAssessmentId, name: url, url };
        }
      });
...
const downloadAssessmentReport = async (workspaceAssessmentId) => {
  ...
  const fileName = generateSafePdfFilename(
    assessment.name || assessment.report.title || "assessment-report"
  );
