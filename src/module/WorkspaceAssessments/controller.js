...
const getWorkspaceAssessment = catchAsync(async (req, res) => {
  let workspaceAssessment = await workspaceAssessmentService.getWorkspaceAssessmentById(
...
const downloadAssessmentReport = catchAsync(async (req, res) => {
  ...
  const workspaceAssessment =
    await workspaceAssessmentService.downloadAssessmentReport(workspaceAssessmentId);
  if (!workspaceAssessment.status) {
    throw new ApiError(httpStatus.NOT_FOUND, workspaceAssessment.message);
  }
  ...
  res.setHeader(
    "Content-Disposition",
    `attachment; filename="${workspaceAssessment.data.fileName}"`
  );
