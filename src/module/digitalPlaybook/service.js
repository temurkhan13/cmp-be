    ...
    if (["Discovery", "Adopt", "Deploy", "Design", "Run"].includes(stageName)) {
      nodeData = transformSpecificNodeData(
        Object.entries(messageObj)
          .filter(([_key, value]) => typeof value === "string" && value.length > 0)
...
const create = async (sitemapBody) => {
  try {
    const { data: playbook, error: insertError } = await supabase
      ...
      .insert({
        name:
          sitemapBody.message?.substring(0, 100) || sitemapBody.sitemapName || "Initial Sitemap",
    ...
    logger.error("Sitemap/playbook error:", error.message, error.response?.data || "");
...
const querySitemaps = async (filter, options) => {
  return await paginate(
    "digital_playbooks",
    { ...options, filter: { ...filter, is_soft_deleted: false } },
    supabase
  );
};
...
const deleteSitemap = async (id) => {
  ...
  await supabase.from("digital_playbooks").update({ is_soft_deleted: true }).eq("id", id);
...
const updateSitemapFields = async (id, updateBody) => {
  try {
    ...
    logger.error("Error updating sitemap fields:", error.message);
...
const updateNodeData = async (playbookId, stageId, nodeId, nodeDataId, updateBody) => {
  ...
  if (nodeDataId) {
    ...
    const { error } = await supabase.from("playbook_nodes").update(updateFields).eq("id", nodeId);
    if (error) throw new ApiError(httpStatus.BAD_REQUEST, error.message);
  }
...
const updateType = async (playbookId, stageId, type, typeId, updateBody) => {
  ...
  if (type === "nodes") {
    ...
    await supabase.from("playbook_stage_node_data").update(updateFields).eq("id", typeId);
  }
...
const inspire = async (inspireBody) => {
  try {
    ...
    logger.error("Inspire error:", error.message, error.response?.data || "");
