const {
	...
	AlignmentType, BorderStyle, Footer, Table, TableRow, TableCell,
...
const htmlToDocxParagraphs = (html) => {
	...
	const flushBlockquote = () => {
		...
		if (text) {
			paragraphs.push(new Paragraph({
				children: [new TextRun({ text: clean(text), italics: true, font: FONT, size: TEXT_SIZE, color: COLOR.textMedium })],
	...
	const flushTable = () => {
		...
		try {
			...
			const headerRow = tableHeaders.length > 0 ? new TableRow({
				tableHeader: true,
				children: tableHeaders.map((h) => new TableCell({
					children: [new Paragraph({
						children: [new TextRun({ text: clean(h), bold: true, font: FONT, size: 18, color: "FFFFFF" })],
			...
			const dataRowObjs = tableRows.map((row, idx) => new TableRow({
				children: row.map((cell) => new TableCell({
					children: [new Paragraph({
						children: [new TextRun({ text: clean(cell), font: FONT, size: 18, color: COLOR.textDark })],
			...
			for (const row of allRows) {
				paragraphs.push(new Paragraph({
					children: [new TextRun({ text: row.map(clean).join("  |  "), font: FONT, size: TEXT_SIZE, color: COLOR.textDark })],
					spacing: { after: 40 },
				}));
			}
		}
	...
	const parseInlineToRuns = (html) => {
		...
		for (const part of parts) {
			...
			if (lower === "<code>") { mono = true; continue; }
			if (lower === "</code>") { mono = false; continue; }
	...
	for (const rawLine of lines) {
		...
		// ── Blockquote ──
		if (line.startsWith("<blockquote>")) { inBlockquote = true; continue; }
		if (line.includes("</blockquote>")) { inBlockquote = false; flushBlockquote(); continue; }
		if (inBlockquote) {
		...
		// ── Table ──
		if (line.startsWith("<table>")) { inTable = true; tableHeaders = []; tableRows = []; continue; }
		if (line === "</table>") { inTable = false; flushTable(); continue; }
		if (inTable) {
			if (line.includes("<th>")) {
				tableHeaders = line.replace(/<\/?tr>/g, "").replace(/<\/?thead>/g, "")
					.split(/<\/?th>/).filter((s) => s.trim() && !s.startsWith("<")).map((s) => s.replace(/<[^>]*>/g, "").trim());
			} else if (line.includes("<td>")) {
				const cells = line.replace(/<\/?tr>/g, "").replace(/<\/?tbody>/g, "")
					.split(/<\/?td>/).filter((s) => s.trim() && !s.startsWith("<")).map((s) => s.replace(/<[^>]*>/g, "").trim());
		...
		if (line.startsWith("<li>")) {
			...
			if (inOl) {
				...
				paragraphs.push(new Paragraph({
					children: [
						new TextRun({ text: `${olIndex}. `, bold: true, font: FONT, size: TEXT_SIZE, color: COLOR.primary }),
						...runs,
					],
					spacing: { after: 60 },
					indent: { left: 500 },
		...
		if (line.match(/^<h3[> ]/)) {
			...
			paragraphs.push(new Paragraph({
				children: [new TextRun({ text, bold: true, font: FONT, size: 28, color: COLOR.textDark })],
		...
		if (line.match(/^<h[4-6][> ]/)) {
			...
			paragraphs.push(new Paragraph({
				children: [new TextRun({ text, bold: true, font: FONT, size: 24, color: COLOR.textMedium })],
		...
		if (line.match(/^<\/?(div|section|article|header|footer|nav|aside|main|figure|figcaption|pre|code)/)) continue;
		...
		children: [
			...
			new Paragraph({
				children: [new TextRun({
					text: normalized.metadata?.source === "playbook" ? "DIGITAL PLAYBOOK" : "ASSESSMENT REPORT",
					font: FONT, size: 22, color: COLOR.textLight, characterSpacing: 200,
				})],
			...
			new Paragraph({
				children: [new TextRun({
					...
					bold: true, font: FONT, size: 56, color: COLOR.primary,
				})],
				alignment: AlignmentType.CENTER,
				spacing: { after: 200 },
			}),
			...
			...(normalized.metadata?.workspaceName ? [
				new Paragraph({
					children: [new TextRun({ text: normalized.metadata.workspaceName, font: FONT, size: 20, color: COLOR.textMedium })],
					alignment: AlignmentType.CENTER,
				}),
			] : []),
			new Paragraph({ spacing: { before: 2000 } }),
			new Paragraph({
				children: [new TextRun({ text: "Generated by ChangeAI", font: FONT, size: 18, color: COLOR.textLight, italics: true })],
				alignment: AlignmentType.CENTER,
			}),
		],
	};
				...
				footers: {
					default: new Footer({
						children: [
							new Paragraph({
								children: [
									new TextRun({ text: "Generated by ChangeAI", font: FONT, size: 16, color: COLOR.textLight, italics: true }),
								],
								alignment: AlignmentType.CENTER,
								border: { top: { style: BorderStyle.SINGLE, size: 1, color: COLOR.border, space: 4 } },
							}),
						],
					}),
				},
