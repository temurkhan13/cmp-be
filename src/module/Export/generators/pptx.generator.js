...
const parseInline = (html, baseFontSize) => {
  ...
  for (const part of parts) {
    ...
    if (!text || text === " ") {
      if (objs.length > 0 && text === " ")
        objs.push({ text: " ", options: { fontSize: sz, color: COLOR.textDark } });
      continue;
    }
...
const htmlToSlideItems = (html) => {
  ...
  for (const rawLine of lines) {
    ...
    if (line.startsWith("<li>")) {
      ...
      if (inOl) {
        olIndex++;
        items.push({ type: "numberedItem", num: olIndex, textObjs });
      } else {
        items.push({ type: "bulletItem", textObjs });
      }
      continue;
    }
    ...
    // ── Headings ──
    if (line.match(/^<h1[> ]/)) {
      items.push({ type: "h1", text: stripTags(line) });
      continue;
    }
    if (line.match(/^<h2[> ]/)) {
      items.push({ type: "h2", text: stripTags(line) });
      continue;
    }
    if (line.match(/^<h3[> ]/)) {
      items.push({ type: "h3", text: stripTags(line) });
      continue;
    }
    if (line.match(/^<h[4-6][> ]/)) {
      items.push({ type: "h4", text: stripTags(line) });
      continue;
    }
    ...
    if (
      line.match(
        /^<\/?(blockquote|pre|code|div|section|figure|figcaption|header|footer|nav|aside|main)/
      )
    )
      continue;
    ...
    if (text)
      items.push({
        type: "paragraph",
        textObjs: [{ text, options: { fontSize: FS.body, color: COLOR.textDark } }],
      });
  }
...
const generate = async (normalized) => {
  ...
  const newSlide = (headerText) => {
    ...
    if (headerText) {
      ...
      currentSlide.addShape(pptx.ShapeType.line, {
        x: SLIDE.marginX,
        y: SLIDE.headerY + 0.55,
        w: SLIDE.textW,
        h: 0,
        line: { color: "DCDCDC", width: 0.5 },
      });
  ...
  const ensureSpace = (needed) => {
    if (!currentSlide || yPos + needed > SLIDE.maxContentY) {
      newSlide(
        currentSectionHeading ? currentSectionHeading + (currentSlide ? " (cont.)" : "") : ""
      );
    }
  };
    ...
    switch (item.type) {
      ...
      case "bulletItem": {
        ...
        const bulletObjs = item.textObjs.map((o) => ({
          ...o,
          options: { ...o.options, fontSize: FS.bullet, bullet: { code: "2022" }, indentLevel: 0 },
        }));
        currentSlide.addText(
          bulletObjs.length > 0
            ? bulletObjs
            : [{ text: fullText, options: { fontSize: FS.bullet, bullet: { code: "2022" } } }],
          {
      ...
      case "numberedItem": {
        ...
        const numObjs = [
          {
            text: `${item.num}. `,
            options: { fontSize: FS.bullet, bold: true, color: COLOR.primary },
          },
          ...item.textObjs,
        ];
          ...
          rows.push(
            row.map((cell) => ({
              ...
              options: {
                ...
                color: COLOR.textDark,
                fill: idx % 2 === 0 ? { color: "F5F7FA" } : undefined,
                align: "left",
                margin: [3, 5, 3, 5],
              },
            }))
