...
const htmlToRows = (html) => {
  ...
  for (const rawLine of lines) {
    ...
    if (
      ...
      line.startsWith("<ol") ||
      line.startsWith("<li") ||
      line.match(/^<\/?(blockquote|pre|code|div|section|figure)/)
    )
      continue;
...
const generate = async (normalized) => {
  ...
  for (const row of dataRows) {
    ...
    if (row.isTable) {
      ...
      for (const tRow of tRows) {
        ...
        dRow.eachCell({ includeEmpty: true }, (cell) => {
          ...
          cell.border = THIN_BORDER;
          if (isStripe)
            cell.fill = { type: "pattern", pattern: "solid", fgColor: { argb: COLOR.stripeBg } };
        });
        rowIdx++;
      }
      continue;
    ...
    dataRow.eachCell({ includeEmpty: true }, (cell, colNumber) => {
      cell.font = {
        ...
        name: "Calibri",
        ...(colNumber === 1 && row.section !== prevSection
          ? { bold: true, color: { argb: COLOR.primary } }
          : {}),
      };
      cell.alignment = { vertical: "top", wrapText: true, horizontal: "left" };
      cell.border = THIN_BORDER;
  ...
  for (const row of dataRows) {
    ...
    tSheet.autoFilter = {
      from: { row: 1, column: 1 },
      to: { row: 1 + row.tableRows.length, column: colCount },
    };
